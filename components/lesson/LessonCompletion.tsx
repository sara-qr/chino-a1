...
export function LessonCompletion({ number, vocabulary, pronunciation, score, maxScore, practiceDone, courseProgress, storageError, onReviewSession }: {
  number: number; vocabulary: string; pronunciation: string; score: number; maxScore: number;
  practiceDone: boolean; courseProgress: number; storageError: string; onReviewSession: () => void;
}) {
  return <div className="rounded-[2.5rem] bg-[#10284F] p-8 text-[#F6F1E8] sm:p-12">
    ...
    <div className="mt-8 flex flex-wrap gap-3"><button type="button" onClick={onReviewSession} className={`${buttonStyle} bg-[#F6F1E8] text-[#10284F] focus-visible:outline-white`}>Revisar sesión</button><Link href="/course" className={`${buttonStyle} bg-[#F6F1E8] text-[#10284F] focus-visible:outline-white`}>Volver al curso</Link><Link href="/dashboard" className={`${buttonStyle} border border-white/50 focus-visible:outline-white`}>Continuar más tarde</Link></div>
  </div>;
}
